...
(function ($, window) {
    ...
    functionality.template = [
        ...
        '<input type="text" class="input-search {className}" value="" placeholder="{placeholder}" />',
        '</div>',
        '</div>',
    ...
    functionality.multiAchieve = function (event) {
        ...
        //存储选中的值
        self.values = [];
        self.multiSelectResult.length > 0 ? self.values.push({
            'name': self.multiSelectResult,
            'id': self.multiSelectResultId,
            'parentId': self.provinceId
        }) : '';
    ...
    Cityselect.prototype.createSubject = function () {
        ...
        template = template.replace('{className}', configure.addInputClass);

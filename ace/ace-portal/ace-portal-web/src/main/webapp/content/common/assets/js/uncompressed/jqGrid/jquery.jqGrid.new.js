...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class=' ui-th-"+dir+"'"+ tooltip+">";
		...
		$(this).addClass('table table-striped table-bordered table-hover dataTable no-footer dtr-inline').append(firstr);

		this.appendChild(tbody);

		console.log(thead);

		$("#"+$.jgrid.jqID(ts.p.id)+" tbody").before(thead);
		firstr = null;
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if(scl){
							...
							if(opt.title){
                                $("td:eq(0)",trdata[0]).attr("colspan",4);
                                $("td:eq(1)",trdata[0]).remove();
                                $("td:eq(2)",trdata[0]).remove();
                                $("td:eq(3)",trdata[0]).remove();
                                $("td:eq(0)",trdata[0]).html("<h5 class='header-title-jqgrid'>"+obj.p.colNames[i]+"</h5>");
                                $("td:eq(0)",trdata[0]).css("text-align","left");
                                $("td:eq(0)",trdata[0]).css("padding-left","12px");
                                t++;
                                cnt++;

...
function initUploader(browseBtn, id) {
    var uploader = new plupload.Uploader({
        ...
        url: contextPath + '/www/appealCase/uploadFile.do?appealCaseId=' + id + '&domain=' + fastdfs_server,
        ...
        if (!rst.state) {
            ...
            $.each(rst.value, function (n, obj) {
                html.push('<div id="' + obj.id + '"> ' +
                    '<a href="' + obj.mediUrl + '" target="_blank">' + obj.name + '</a> (' + parseInt(file.size / 1024) + 'kb)' +
                    ' <a class=\'ace-icon glyphicon glyphicon-remove bigger-110\' href="javascript:deleteAttach(\'' + obj.id + '\',\'' + obj.appealCaseId + '\')"></a><b></b></div>');
            });

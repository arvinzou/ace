$(function () {
    ...
    juicer.register('scored', scored);
    juicer.register('thisIsRight', thisIsRight);
    juicer.register('isRight', isRight);
...
function scored(data) {
    var ops=data.topicOptRstList;
    for(item in ops) {
        if (ops[item].youAnswer != ops[item].answer) {
            return 0;
        }
    }
    return data.tscore
...
function thisIsRight(data){
	var s='rightOption'
	for(item in data){
		if(data[item].youAnswer!=data[item].answer){
			s='failOption'
		}
	}
	return s;
...
function isRight(data){
	if(data.answer==1&&data.youAnswer==0){
		return 'right';
	}
	if(data.answer==1&&data.youAnswer==1){
		return 'fullRight';
	}
	if(data.answer==0&&data.youAnswer==1){
		return 'fail';
	}
	return '';
}

...
function jcalendar_week(options){
	...
	var setdaydata = function(year,weeknum){
		...
		var month = parseInt(datastart[1])+1;
		calendarid.find(".calendar_info").html(trueweeknum[1]+'周/'+trueweeknum[0]+"年"+month+"月");
		...
		for(var i=0;i<7;i++){
			...
			if(newdate.getFullYear()==todayarr[0] && newdate.getMonth()==todayarr[1] && newdate.getDate()==todayarr[2]){
				week_day+='<li class="calendar_day calendar_day_act'+dayaddclass+'" '+

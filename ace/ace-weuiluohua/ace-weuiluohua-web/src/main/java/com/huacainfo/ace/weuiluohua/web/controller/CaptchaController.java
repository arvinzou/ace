...
@Controller
@RequestMapping("/www/captcha")
public class  CaptchaController extends WeuiBaseController {
	...
	public void image(HttpServletRequest request, HttpServletResponse response)
		...
		//在画板上写数字，起始位置
		    g.drawString(rand,25*i+30,32);
		}
		// 保存进session
		this.getRequest().getSession().setAttribute("j_captcha_weuiluohua",sRand);
		logger.debug("j_captcha_weuiluohua:{}",this.getRequest().getSession().getAttribute("j_captcha_weuiluohua"));
		// 显示图片
		g.dispose();
		//转换成一张图片，格式为JPEG

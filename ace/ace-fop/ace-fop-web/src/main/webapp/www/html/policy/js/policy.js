...
app.controller(ngControllerName,function($scope){
    $.ajax({
        ...
        type:"post",
        async:false,
        data:{limit:pageSize, page: currentPage, modules: "5"},  //5代表政策文件
    ...
    $scope.searchList = function(currentPage, pageSize){
        $.ajax({
            ...
            type:"post",
            async:false,
            data:{limit:pageSize, page: currentPage, modules: "5"},
     ...
     * 政策文件下载
     */
    $scope.download = function(file, title){
        if(file != '' && file != null){
            document.getElementById("download").setAttribute("download",title);
            document.getElementById("download").href="http://zx.huacainfo.com/"+file;
        }else{

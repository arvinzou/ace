...
app.controller(ngControllerName,function($scope){
    ...
    $.ajax({
        ...
        type:"post",
        async:false,
        data:{"limit":pageSize, page: currentPage, noticeType: "1"},
    ...
    $scope.search = function(){
        ...
        $.ajax({
            ...
            type:"post",
            async:false,
            data:{"limit":pageSize, page: 1, title : key_word,noticeType: "1"},
            success:function(result){
                if(result.status == 0) {
                    $scope.items = result.data.list;
                    if (!$scope.$$phase) {
    ...
    $scope.sortList = function(name){
        ...
        $.ajax({
            ...
            type:"post",
            async:false,
            data:{"limit":pageSize,  page: 1, sord : orderParam, noticeType: "1"},
            success:function(result){
                if(result.status == 0) {
                    $scope.items = result.data.list;
                    if (!$scope.$$phase) {
    ...
    $scope.searchList = function(currentPage, pageSize){
        ...
        $.ajax({
            ...
            type:"post",
            async:false,
            data:{page:currentPage, limit: pageSize, title: key_word, noticeType: "1"},

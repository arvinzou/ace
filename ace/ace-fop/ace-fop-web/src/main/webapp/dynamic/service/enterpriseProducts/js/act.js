...
function getPageList() {
    var url = contextPath + '/informationService/findInformationServiceList';
...
function findDetail(id){
    startLoad();
    $.ajax({
        url: contextPath + '/informationService/selectInformationServiceByPrimaryKey',
        type:"post",
        async:false,
        data:{id: id},
        success:function(rst){
            if(rst.status == 0){
                stopLoad();
                render($('#fm-preview'), rst.value, 'tpl-preview');
                initPhotoPreview(".my-gallery img");
            }else{
                alert(rst.errorMessage);
            }

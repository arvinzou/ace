...
$(function () {
    ...
    // initpdf();
    initClock();
    juicer.register('substringDate', substringDate);
...
function substringDate(dateStr) {
            ...
            for (var i = 0; i < datas.length; i++) {
                ...
                if (item.fileUrl) {
                    if (item.fileUrl.lastIndexOf('.pdf') != -1) {
                        ...
                        initpdf('http://api.huacainfo.com/group1/M00/00/01/ChEHaVyhsKyAEmKVAAJ4vGnD2ok117.pdf?filename=11楼座次表__第四期辅警（桃源）.pdf', i);
                        continue;
...
function getCourseList() {
    ...
    $.getJSON(url, data, function (rst) {
        if (rst.status == 0) {
            var datas = rst.data.reverse();
            renderPage('classList', datas, 'tpl-classList');
        }
    });
}

...
function initDataMap(isInit, mapType) {
	$.ajax({
		type : "post",
		url : contextPath+"/gpsChianDataznsh/selectListByPAreaName01.do",
		data : {
			areaName : areaName
		},
...
function initDataBar(isInit) {
	$.ajax({
		type : "post",
		url : contextPath+"/gpsChianDataznsh/selectListByPAreaName03.do",
		data : {
			areaName : areaName
		},
		success : function(rst, textStatus) {
			...
			optionBar.title.text = areaName + "审核违规分布情况统计(单位：万元)";
...
function initDataPieolds(isInit){
	$.ajax({
		type : "post",
		url : "/portal/gpsChianDataznsh/selectListByPAreaName05.do",
		data : {
			areaName : areaName
		},
		success : function(rst, textStatus) {
...
function initDataTableJzdx() {
	...
	$.ajax({
		type : "post",
		url : contextPath+"/gpsChianDataznsh/selectListByPAreaName12.do",
		data : {
			areaName : areaName,
			page : page
		},
		success : function(rst, textStatus) {
			...
			$.each(rst.data1,function(i,obj){
				...
				html.push('<td><a href="javascript:setCurrentParamJzdx(\''+obj.jbmc+'\')">'+obj.jbmc.substr(0,6)+'</a></td>');
				//html.push('<td align="right" class="tr4">'+obj.jbmc.substr(0,6)+'</td>');
...
function initDataBarJb(isInit) {
	$.ajax({
		type : "post",
		url : contextPath+"/gpsChianDataznsh/selectListByPAreaName14.do",
		data : {
			areaName : areaName
		},
		success : function(rst, textStatus) {
			...
			optionBarJb.title.text = areaName +"审核违规趋势统计";

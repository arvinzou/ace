...
function audit(id,status,rptId){
        $.ajax({
            type : "post",
...
function add() {
	var data={
        id:"ieu44009999",
        rptId:"c15f484b-bd30-4111-904d-123ca617180e",
        uid:"oFvIjw8x1--0lQkUhO1Ta3L59o3c",
        content:"大家好",
        status:"1"
	}
	data.status=rsd(data.status, "113");
	data.opt=renderOpt(data.id,data.rptId,data.rptId);
	rowadd(data);
}
function renderOpt(id,title,rptId) {
	var html = [];
	html.push('<a data-rel="tooltip" data-placement="top" title="通过" class="blue" href="javascript:audit(\'' + id+ '\',2,\'' + rptId+'\')"><i class="ace-icon fa fa-unlock bigger-130"></i></a>');
...
if('WebSocket' in window){
  websocket1 = new ReconnectingWebSocket("ws://"+websocketurl+"/jxb/websocketsub/rmsg/sys");
}
else{

...
$(function () {
    ...
    /*点击发布报道*/
    $('#jxbReportTable').on('click', '.publication', startPublicationDo);

    $('#jxbReportTable').on('click', '.preview-report', startPreviewDo);
    /*根据状态进行查询*/
    ...
    $('#jxbReportTable').on('click', '.Topcheckbox', function (e) {
...
function delectRptDo() {
    $('#jxbReportTable .Topcheckbox input').each(function () {
...
//     if ('全选' == text) {
//         $(this).text('取消全选');
//         $('#jxbReportTable .Topcheckbox input').prop({checked: true});
//     } else if ('取消全选' == text) {
//         $(this).text('全选');
//         $('#jxbReportTable .Topcheckbox input').prop({checked: false});
//     }
//     return false;
// }
...
function actionModifyDo() {
    ...
    $('#htmlLoad').load('./../jsp/floatTable.jsp', function () {
        $('#floatTable').load('./../jsp/modifyRpt.jsp', function () {
            $('#JSLoad').load('./../jsp/modifyRptJS.jsp', function () {
...
function modifyStatus(id,rid,status){
    var url = '/jxb/jxbRpt/updateLiveRptStatus.do';
    var data = {
        'id': id,
        'rid': rid,
        'status': status,
...
function viewReportList(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        $('#jxbReportTable').append($liReport);
    }
}
...
function startPreviewDo() {
    ...
    $.getJSON(url, data, function (result) {
        ...
        if (result.status == 0) {
            $('#htmlLoad').load('./../jsp/floatTable.jsp', function () {
                $('#floatTable').load('./../jsp/previewRptWeb.jsp', function () {

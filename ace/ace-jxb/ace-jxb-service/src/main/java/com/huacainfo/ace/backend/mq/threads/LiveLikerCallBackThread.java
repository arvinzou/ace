...
public class LiveLikerCallBackThread extends Thread {
    private static final Logger LOGGER = LoggerFactory.getLogger(LiveLikerCallBackThread.class);

    private KafkaStream<byte[], byte[]> stream = null;
    private WWWService wwwService;

    public LiveLikerCallBackThread(String name, KafkaStream<byte[], byte[]> stream) {
        super(name);
        this.stream = stream;
        this.init();
    }

    public LiveLikerCallBackThread(ThreadGroup group, String name, KafkaStream<byte[], byte[]> stream) {
        super(group, name);
        this.stream = stream;
        this.init();
    }
